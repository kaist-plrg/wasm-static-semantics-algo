...
let type_of_num_pat = function
  ...
  | NanPat op -> Value.dtype_of_op op.it
...
let assert_return ress ts at =
  ...
  let rec test (res, t) =
    ...
    | NumResult (NumPat {it = num; at = at'}) ->
      let t', reinterpret = reinterpret_of (Value.dtype_of_op num) in
      [ reinterpret @@ at;
        Const (num @@ at')  @@ at;
        reinterpret @@ at;
        Compare (eq_of t') @@ at;
    ...
    | NumResult (NanPat nanop) ->
      ...
      in
      let t', reinterpret = reinterpret_of (Value.dtype_of_op nanop.it) in
      [ reinterpret @@ at;
